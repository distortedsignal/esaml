...
verify(Element, Fingerprints) ->
    ...
    #xmlAttribute{value = SignatureMethodAlgorithm} = hd(xmerl_xpath:string("//ds:Signature/ds:SignedInfo/ds:SignatureMethod/@Algorithm", Element, [{namespace, DsNs}])),
    {HashFunction, _, _} = signature_props(SignatureMethodAlgorithm),

    #xmlAttribute{value = "http://www.w3.org/2001/10/xml-exc-c14n#"} = hd(xmerl_xpath:string("//ds:Signature/ds:SignedInfo/ds:CanonicalizationMethod/@Algorithm", Element, [{namespace, DsNs}])),
    #xmlAttribute{value = SignatureMethodAlgorithm} = hd(xmerl_xpath:string("//ds:Signature/ds:SignedInfo/ds:SignatureMethod/@Algorithm", Element, [{namespace, DsNs}])),
    C14nTx = hd(xmerl_xpath:string("//ds:Signature/ds:SignedInfo/ds:Reference/ds:Transforms/ds:Transform[@Algorithm='http://www.w3.org/2001/10/xml-exc-c14n#']", Element, [{namespace, DsNs}])),
    InclNs = case xmerl_xpath:string("//ec:InclusiveNamespaces/@PrefixList", C14nTx, [{namespace, DsNs}]) of
        [] -> [];
        AttrList when is_list(AttrList) ->
            #xmlAttribute{value=NsList} = hd(AttrList),
    ...
    #xmlText{value = Sha64} = hd(xmerl_xpath:string("//ds:Signature/ds:SignedInfo/ds:Reference/ds:DigestValue/text()", Element, [{namespace, DsNs}])),
    ...
    true ->
        SigInfo = hd(xmerl_xpath:string("//ds:Signature/ds:SignedInfo", Element, [{namespace, DsNs}])),
        SigInfoCanon = xmerl_c14n:c14n(SigInfo),
        Data = list_to_binary(SigInfoCanon),

        #xmlText{value = Sig64} = hd(xmerl_xpath:string("//ds:Signature//ds:SignatureValue/text()", Element, [{namespace, DsNs}])),
        Sig = base64:decode(Sig64),

        #xmlText{value = Cert64} = hd(xmerl_xpath:string("//ds:Signature//ds:X509Certificate/text()", Element, [{namespace, DsNs}])),
